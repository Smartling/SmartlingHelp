---
title: Authenticate
categories:
  - authentication
layout: v2endpoint
endpoint_url: /auth-api/v2/authenticate
method: POST
request:
  json_markdown: |
    ~~~json
    {
        "userIdentifier": "gwvnhsaiwtvblovagroolsqgrrlymz",
        "userSecret": "g310k2ekrvg7arn27turqqnu6uMJ.ep0n3o3oo3nteiq76impr8hr69"
    }
    ~~~
  parameters:
    - title: userIdentifier
      type: string
      required: true
      description_markdown:
        Your User ID.
    - title: userSecret
      type: string
      required: true
      description_markdown:
        User secret for the token ID.

response:
  json_markdown: |
    ~~~json
    {
      "response": {
        "code": "SUCCESS",
        "data": {
          "accessToken": "eyJhbGciOiJSUzI1NiJ9..e5z7taFefXx87TOhte2asn6b-GLb0mweJW6fCyxlhLXw_4Cf_k4R_4iEo2ZuSS9-6s__C3M5I__q77MtHBQoJjML8-rrvA4-rdlWu2XC5IraKX-2XmZnmWxYTZUvZ0Y82dC_Kdgv7cSaLds5hAEa1MfsC_tTsb0XR_gCCEcOWqg",
          "refreshToken": "eyJhbGciOiJSUzI1NiJ9..fgWAVI66vO7UK9DfMEf8Azna-mVECT9eSMYe3Rr6Bf73Ux56UO-X2YZGgHn8mzfWFi9LPFDZjp1kossmePp_cG2ZAqDdtZ_zWqp1HCipz955mNLAPITVrpyrAAEqCFO59qrAaGDd6Yi9gFhZNKwe4xigSdOQ2szXTvKAdqu7vKc",
          "expiresIn": 480,
          "refreshExpiresIn": 3660,
          "tokenType": "Bearer"
        }
      }
    }
    ~~~
  parameters:
    - title: accessToken
      type: string
      description_markdown: Token to authenticate all other API requests.
    - title: expiresIn
      type: string
      description_markdown: Number of seconds until access token expires.
    - title: refreshToken
      type: string
      description_markdown: Token for a Refresh Authentication request.
    - title: refreshExpiresIn
      type: string
      description_markdown: Number of seconds until refresh token expires.
    - title: tokenType
      type: string
      description_markdown: Type of access token being returned. Currently, this will always be `Bearer`.
examples:
  - title: Curl
    code_markdown: |
      ~~~curl
      curl -X POST -H "Content-Type: application/json" -d '{
          "userIdentifier": "{identifier}",
          "userSecret": "{secret}"
      }' 'https://api.smartling.com/auth-api/v2/authenticate'
      ~~~
    description_markdown:
---

Authenticates the user with a User Identifier and Secret. Returns an access token that can be attached to the header of any API request to authenticate the user, and a refresh token which can be used to obtain a new access token. See [Authentication](/developers/api/v2/authentication/) for more information.